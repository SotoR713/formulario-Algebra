...
// ===== EXPONENTES =====
import {
  multiplicarPotencias,
  dividirPotencias,
  potenciaDePotencia,
  exponenteNegativo
} from "./modules/exponentes.js";
...
import {
  ...
  rectaDosPuntos,
  rectaPuntoPendiente
} from "./modules/rectas.js";

// ==========================
// DOM
// ==========================
const tema = document.getElementById("tema") as HTMLSelectElement;
...
function resolver() {
  ...
  const str = (id: string, d: string) => (document.getElementById(id) as HTMLInputElement).value || d;
...
// RESULTADOS
// ==========================
function mostrarPasos(pasos: Paso[]) {
  pasos.forEach((p, i) => {
...
function limpiarTodo() {
  resultadoDiv.innerHTML = "";
  const c = document.getElementById("grafica") as HTMLCanvasElement;
  c?.getContext("2d")?.clearRect(0, 0, c.width, c.height);
}


function dibujarPendiente(m: number) {
...
function dibujarRecta(m: number, b: number) {
  const canvas = document.getElementById("grafica") as HTMLCanvasElement;
  if (!canvas) return;
