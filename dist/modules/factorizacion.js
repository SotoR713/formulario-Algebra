 ...
 * a² - b²
 */
export function diferenciaDeCuadrados(a, b) {
 ...
 * a³ - b³
 */
export function diferenciaDeCubos(a, b) {
    const aNum = esNumero(a);
    const bNum = esNumero(b);
    const pasos = [];
    pasos.push({
    ...
    if (aNum && bNum) {
        pasos.push({
            descripcion: "Resultado final",
            expresion: `(${Number(a) - Number(b)})(${Math.pow(Number(a), 2) + Number(a) * Number(b) + Math.pow(Number(b), 2)})`
        });
    }
    return pasos;
}
 ...
 * a³ + b³
 */
export function sumaDeCubos(a, b) {
    const aNum = esNumero(a);
    const bNum = esNumero(b);
    const pasos = [];
    pasos.push({
 ...
 */
export function trinomioCuadratico(b, c) {
    const pasos = [];
    pasos.push({
